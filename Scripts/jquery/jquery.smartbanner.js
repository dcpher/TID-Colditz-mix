...
!function ($) {
    var SmartBanner = function (options) {
        ...
        var standalone = navigator.standalone // Check if it's already a standalone web app or running within a webui view of an app (not mobile safari)
          , UA = navigator.userAgent; // Detect banner type (iOS or Android)
        ...
        if (this.scale < 1) this.scale = 1; // Get info from meta data
        var meta = $(this.type == 'android' ? 'meta[name="google-play-app"]' :
            this.type == 'ios' ? 'meta[name="apple-itunes-app"]' : 'meta[name="msApplication-ID"]');
        if (meta.length == 0) return; // For Windows Store apps, get the PackageFamilyName for protocol launch
        ...
        this.author = this.options.author ? this.options.author : ($('meta[name="author"]').length ? $('meta[name="author"]').attr('content') : window.location.hostname); // Create banner
        this.create();
        this.show();
        this.listen();
    };
    ...
    $.smartbanner.defaults = {
        ...
        appStoreLanguage: 'au', // Language code for App Store
